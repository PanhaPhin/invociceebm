...
{
    /**
     * Create PayPal order for the given invoice.
     *
    ...
    public function onBoard(Request $request)
    {
        ...
        $currencyCode = strtoupper(getInvoiceCurrencyCode($invoiceCurrencyId));

        if (! in_array($currencyCode, getPayPalSupportedCurrencies())) {
            return $this->sendError("Currency {$currencyCode} is not currently supported.", 400);
        }
