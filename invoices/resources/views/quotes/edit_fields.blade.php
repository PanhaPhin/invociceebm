...
<div class="row">
    ...
    <div class="col-lg-4 col-sm-12 mb-5">
        ...
        {{ Form::text('quote_date', null, ['class' => 'form-control', 'id' => 'editQuoteDate', 'autocomplete' => 'off', 'required']) }}
    </div>

    <div class="col-lg-4 col-sm-12 mb-5">
        {{ Form::label('due_date', __('messages.quote.due_date').(':'), ['class' => 'form-label required mb-3']) }}
        {{ Form::text('due_date', null, ['class' => 'form-control', 'id' => 'editQuoteDueDate', 'autocomplete' => 'off', 'required']) }}
    </div>
    ...
    <div class="col-12 text-end mb-lg-10 mb-6">
        <button type="button" class="btn btn-primary" id="addQuoteItem">{{ __('messages.invoice.add') }}</button>
    </div>
        ...
        <table class="table table-striped box-shadow-none mt-4" id="billTbl">
            <thead>
                <tr class="border-bottom fs-7 fw-bolder text-gray-700 text-uppercase">
                    ...
                    {{-- <th scope="col" class="required">{{ __('messages.product.description') }}</th>
                    <th scope="col" class="required">{{ __('messages.quotation.rate') }}</th> --}}
                ...
                @foreach($quote->quoteItems as $quoteItem)
                    <tr class="tax-tr">
                        ...
                        {{-- <td class="table__item-desc">
                            {{ Form::text('description[]', $quoteItem->description, ['class' => 'form-control description-quote', 'required']) }}
                        </td>
                        <td class="table__item-desc">
                            {{ Form::text('rate[]', $quoteItem->rate, ['class' => 'form-control rate-quote', 'required']) }}
                        </td> --}}
                        <td class="quote-item-total pt-8 text-nowrap">
                            {{ number_format($quoteItem->total, 2) }}
                        </td>
                        <td class="text-end">
                            <button type="button" title="Delete" class="btn btn-icon fs-3 text-danger btn-active-color-danger delete-quote-item">
    ...
    <div class="row">
        <div class="col-lg-7 col-sm-12 mt-2 mt-lg-0">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="text-gray-900 text-end pe-0">
                                <span>{{ getCurrencySymbol() }}</span> <span id="quoteTotal" class="price">{{ isset($quote) ? number_format($quote->amount, 2) : 0 }}</span>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td class="text-gray-900 text-end pe-0">
                                <span>{{ getCurrencySymbol() }}</span> <span id="quoteFinalAmount">{{ isset($quote) ? number_format($quote->amount - ($quote->amount * $quote->discount / 100), 2) : 0 }}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <div class="row justify-content-left">
        ...
        <div class="col-lg-6 mb-0 mt-0" id="quoteNoteAdd">
        ...
        <div class="col-lg-6 mb-5" id="quoteTermRemove">
...
<div class="float-end">
    <div class="form-group col-sm-12">
        <button type="button" name="save" class="btn btn-primary mx-3" id="editSaveQuote" data-status="0" value="0">{{ __('messages.common.save') }}</button>
